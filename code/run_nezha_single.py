...
def get_data_ids(data, labids, is_training=True, is_eda=False):
    ...
    data_his_all = fr_his.readlines()[1:]
    all_history_dict = {}

    for his in data_his_all:
        his = his.strip("\r")
        his = his.strip("\n")
        his_list = his.split("\t")
        id = his_list[0]
    ...
    for i, data_tmp in enumerate(data):
        ...
        while count < his_count:
            if thir_id < min_key:
                break
            if thir_id in his_tmp_dict:
                count += 1
                history_str = his_tmp_dict[thir_id] + history_str
            thir_id -= 1
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        for train_index, dev_index in mskf.split(data_in_z, all_lab_ids):
            ...
            train_in_ids, train_in_masks, train_seg_ids,train_lab_ids = get_data_ids(X_train,y_train,True,False)
            ...
            dev_in_ids, dev_in_masks, dev_seg_ids, dev_lab_ids = get_data_ids(X_dev, y_dev, True)
    ...
    if FLAGS.do_predict:
        ...
        test_in_ids, test_in_masks, test_seg_ids, test_lab_ids = get_data_ids(data_pd_all,test_lab_ids, False)
