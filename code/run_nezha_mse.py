...
flags.DEFINE_integer("random_seed", 71, "random_seed.")
...
def get_data_ids(data_ids, is_training=True, train_flag=False):
    ...
    if is_training:
        fr_his = open(os.path.join(FLAGS.data_dir, "train_dataset_v2.tsv"), "r")
    else:
        fr_his = open(os.path.join(FLAGS.data_dir, "test_dataset.tsv"), "r")
...
def main(_):
    ...
    # head_size = 64
    # test_tf = tf.ones([4,128,768])
    # test_tf = tf.layers.dense(test_tf, 3*64*2)
    # inputs = tf.split(test_tf, 3, axis=-1)
    ...
    #
    # indices = K.arange(0, head_size // 2, dtype=K.floatx())
    # indices = K.pow(10000.0, -2 * indices / head_size)
    # embeddings = tf.einsum('bn,d->bnd', position_ids, indices)
    # embeddings = K.stack([K.sin(embeddings), K.cos(embeddings)], axis=-1)
    # embeddings = K.reshape(embeddings, (-1, seq_len, head_size))
    # pos = embeddings
    #
    ...
    # # scale返回
    # output = logits / head_size ** 0.5
    #
    # bh = K.prod(K.shape(output)[:2])
    # y_true = K.reshape(output, (bh, -1))
    # y_pred = K.reshape(output, (bh, -1))
    # loss = K.mean(multilabel_categorical_crossentropy(y_true, y_pred))


    if FLAGS.do_train:
        train_data_ids_all = get_data_ids_all(os.path.join(FLAGS.data_dir, "train_dataset_v2.tsv"))
    ...
    if FLAGS.do_predict:
        ...
        test_data_ids_all = get_data_ids_all(os.path.join(FLAGS.data_dir, "test_dataset.tsv"))
