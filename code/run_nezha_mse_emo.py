...
def get_data_ids(data_ids, is_training=True, train_flag=False, dev_flag=False):
    #历史数据
    #his_count = 5  # 296
    his_count = 3  # 256
    ...
    test_first_id = data_ids[0].split("_")[0]
    ...
    for his in data_resd_all:
        ...
        if is_training:
            ...
            if id_tmp in test_label_all:
                emo_str = ""
                lab_int_list = test_label_all[id_tmp]
                if np.sum(lab_int_list) > 0:
                    for idxx, ll in enumerate(lab_int_list):
                        if ll > 0:
                            emo_str = emo_str + level_list[ll] + emotion_list[idxx]
    ...
    for i, data_tmp in enumerate(data_resd_all):
        ...
        while count < 4:
        #while len(history_str_before) < 192 or count < 5:
            ...
            if thir_id_min in his_tmp_dict:
                ...
                if cur_history == history_tmp:
                    if len(cur_history_l[1]) > 0:
                        history_str_before = history_str_before + ";" + cur_history_l[1]
                    thir_id_min -= 1
                    continue

                count += 1
                history_str_before = cur_history + cur_history_l[1] + history_str_before

                if train_flag:
                    history_str_before_org = cur_history  + history_str_before_org
        ...
        if history_str_before_org == "":
            history_str_before_org = "无"
        ...
        history_str_before_org = wash_data(history_str_before_org)
        ...
        for pp in plex_str:
            pp_t = tokenizer.tokenize(tokenization.convert_to_unicode(pp))
            plex_token.extend(pp_t)
        ...
        for his in history_str_before_org:
            his_t = tokenizer.tokenize(tokenization.convert_to_unicode(his))
            history_token_before_org.extend(his_t)
        ...
        fw.write(str(len(history_token_before)) + "\t" + str(len(content_token)) + "\n")

        if len(content_token) > 192:
            truncate_seq_pair_new(history_token_before, "", FLAGS.max_seq_length - 192 - 3 - len(plex_token))
            truncate_seq_pair_new(history_token_before_org, "", FLAGS.max_seq_length - 192 - 3 - len(plex_token))
        else:
            truncate_seq_pair_new(history_token_before, "", FLAGS.max_seq_length-len(content_token) -3- len(plex_token))
            truncate_seq_pair_new(history_token_before_org, "", FLAGS.max_seq_length - len(content_token) - 3- len(plex_token))

        left_len = FLAGS.max_seq_length - len(history_token_before) - 3 - len(plex_token)
        truncate_seq_pair(content_token, "", left_len)

        tokens_all = ["[CLS]"] + history_token_before + ["[SEP]"] + plex_token  + content_token + ["[SEP]"]
        segment_id_tmp = [0] * (len(history_token_before) + 2) + [1] * (len(plex_token) + len(content_token) + 1)
        ...
        if train_flag or dev_flag:
            ...
            left_len = FLAGS.max_seq_length - len(history_token_before_org) - 3 - len(plex_token)
            truncate_seq_pair(content_token, "", left_len)

            tokens_all = ["[CLS]"] + history_token_before_org + ["[SEP]"] + plex_token + content_token + ["[SEP]"]
            segment_id_tmp = [0] * (len(history_token_before_org) + 2) + [1] * (len(plex_token) + len(content_token) + 1)
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        for train_index, dev_index in mskf.split(train_data_ids_all):
            ...
            dev_in_ids, dev_in_masks, dev_seg_ids, dev_lab_ids = get_data_ids(dev_ids, True,False,False)

            dev_in_ids_2, dev_in_masks_2, dev_seg_ids_2, dev_lab_ids_2 = get_data_ids(dev_ids, True, False, True)
                ...
                for epoch in range(FLAGS.num_train_epochs):
                    ...
                    for batch in next_batch(FLAGS.train_batch_size, train_in_ids, train_in_masks, train_seg_ids, train_lab_ids):
                        ...
                        if current_step % FLAGS.save_checkpoints_steps == 0 and current_step > 600:
                            ...
                            eval_prop_all = []
                            label_ids_all = []
                            for eval_batch in next_batch(FLAGS.train_batch_size, dev_in_ids_2, dev_in_masks_2, dev_seg_ids_2, dev_lab_ids_2):
                                eval_feed_dict = {input_ids_holder: eval_batch["input_ids"], input_masks_holder: eval_batch["input_masks"],
                                                  segment_ids_holder: eval_batch["segment_ids"], label_ids_holder: eval_batch["label_ids"]}
                            ...
                            eval_acc_ = accuracy(eval_predictions_all,eval_labs)
                            eval_acc_my, eval_f1_my = acc_and_f1(eval_predictions_all,eval_labs)
                            print("eval-- 2 --org:  loss: %.6f, acc: %.6f, rmse: %.6f, rmse_int: %.6f, acc_my %.4f, f1_my %.4f" %
                                  (np.mean(eval_losses), eval_acc_, eval_rmse,eval_rmse_int,eval_acc_my,eval_f1_my))
