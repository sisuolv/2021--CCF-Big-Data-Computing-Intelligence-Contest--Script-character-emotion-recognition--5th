...
flags.DEFINE_integer("lab_pos", 0, "lab_pos.")
flags.DEFINE_integer("num_labels", 1, "num_labels.")


flags.DEFINE_integer("random_seed", 43, "random_seed.")
...
def convert_float_to_int(intput):
    ...
    for tt in intput:
        ...
        if tt_new > 3:
            tt_new = 3
        output.append(tt_new)

    return output

def convert_float_to_int_mse(intput):
    output = []
    for tt in intput:
        tt_new = tt
        if tt_new < 0:
...
def acc_and_f1(pred_y, true_y, eval=True):
    ...
    for i in range(1):
        ...
        for j in range(len(pred_y)):
            pred_tmp.append(pred_y[j])
            true_tmp.append(true_y[j])
...
def rmse_int(pred_y, true_y):
    pred_y = convert_float_to_int_mse(pred_y)
    pred_y = np.array(pred_y)
    true_y = np.array(true_y)
    ...
    for i, data_tmp in enumerate(data_resd_all):
        ...
        if is_training:
            if len(data_tmp_list[3]) == 0:
                continue
            label_id_str_list = data_tmp_list[3].split(",")
            lab = label_id_str_list[FLAGS.lab_pos]
            label_id = int(lab)
        else:
            label_id = 0
        ...
        while len(history_str_before) < 144 or count < 4:
        ...
        # if len(content) > 64:
        #     history_str_after = content[:64] + history_str_after
        # else:
        #     history_str_after = content + history_str_after
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, labels, num_labels, embed_grad_holder,
    ...
    with tf.variable_scope("loss"):
        ...
        #loss_final = tf.reduce_mean(tf.square(labels - logits))
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        for train_index, dev_index in mskf.split(train_data_ids_all):
            ...
            FLAGS.save_checkpoints_steps = int(num_train_steps / 15)
            ...
            label_ids_holder = tf.placeholder(dtype=tf.float32, shape=[None], name="label_ids_holder")
            ...
            with tf.Session() as sess:
                ...
                for epoch in range(FLAGS.num_train_epochs):
                    ...
                    for batch in next_batch(FLAGS.train_batch_size, train_in_ids, train_in_masks, train_seg_ids, train_lab_ids):
                        ...
                        if current_step % FLAGS.save_checkpoints_steps == 0 and current_step > 600:
                            ...
                            eval_rmse_int = rmse_int(eval_predictions_all, eval_labs)
                ...
                # break
            dev_mloss_all.append(eval_rmse)
