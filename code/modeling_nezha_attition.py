...
class BertModel(object):
    ...
    def __init__(self,
        ...
        input_shape = get_shape_list(input_ids_from, expected_rank=2)
        batch_size = input_shape[0]
        seq_length_from = input_shape[1]

        input_shape_to = get_shape_list(input_ids_to, expected_rank=2)
        seq_length_to = input_shape_to[1]

        if input_mask_from is None:
            input_mask_from = tf.ones(shape=[batch_size, seq_length_from], dtype=tf.int32)

        if input_mask_to is None:
            input_mask_to = tf.ones(shape=[batch_size, input_mask_to], dtype=tf.int32)

        token_type_ids_from = None
        token_type_ids_to = None
        if token_type_ids is None:
            use_token_type_my = True
            token_type_ids_from = tf.zeros(shape=[batch_size, seq_length_from], dtype=tf.int32)
            token_type_ids_to = tf.zeros(shape=[batch_size, seq_length_to], dtype=tf.int32)
        else:
            use_token_type_my = True
        ...
        with tf.variable_scope(scope, default_name="bert"):
            with tf.variable_scope("embeddings"):
                ...
                (self.embedding_output_from, self.embedding_output_to, self.embedding_table) = embedding_lookup(
                    input_ids=input_ids_from,
                    input_ids_to=input_ids_to,
                    vocab_size=config.vocab_size,
                    embedding_size=config.hidden_size,
                    initializer_range=config.initializer_range,
                    word_embedding_name="word_embeddings",
                    use_one_hot_embeddings=use_one_hot_embeddings,
                    embed_grad=embed_grad_adv)

                # Add positional embeddings and token type embeddings, then layer
                # normalize and perform dropout.
                self.embedding_output_from, self.embedding_output_to = embedding_postprocessor(
                    input_tensor=self.embedding_output_from,
                    input_tensor_to=self.embedding_output_to,
                    use_token_type=use_token_type_my,
                    token_type_ids=token_type_ids_from,
                    token_type_ids_to=token_type_ids_to,
                    level_type_ids=level_type_ids,
            ...
            with tf.variable_scope("encoder"):
                ...
                attention_mask = create_attention_mask_from_input_mask(
                    input_ids_from, input_mask_to)


                # Run the stacked transformer.
                # `sequence_output` shape = [batch_size, seq_length, hidden_size].
                self.all_encoder_layers = transformer_model(
                    input_tensor_from=tf.saturate_cast(self.embedding_output_from, compute_type),
                    input_tensor_to=tf.saturate_cast(self.embedding_output_to, compute_type),
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint):
    ...
    for var in tvars:
        name = var.name
        # if name.find("bert/embeddings") > -1:
        #     continue
        # if name.find("cls") > -1:
        #     continue
        m = re.match("^(.*):\\d+$", name)
...
def embedding_lookup(input_ids,
    ...
    if input_ids_to.shape.ndims == 2:
        input_ids_to = tf.expand_dims(input_ids_to, axis=[-1])
    ...
    if embed_grad != None:
        embedding_table = embedding_table + embed_grad
    ...
    flat_input_ids_to = tf.reshape(input_ids_to, [-1])
    if use_one_hot_embeddings:
        one_hot_input_ids_to = tf.one_hot(flat_input_ids_to, depth=vocab_size)
        output_to = tf.matmul(one_hot_input_ids_to, embedding_table)
    else:
        output_to = tf.gather(embedding_table, flat_input_ids_to)

    input_shape_to = get_shape_list(input_ids_to)

    output_to = tf.reshape(output_to,
                        input_shape_to[0:-1] + [input_shape_to[-1] * embedding_size])

    return (output, output_to, embedding_table)
...
def embedding_postprocessor(input_tensor,
    ...
    input_shape_to = get_shape_list(input_tensor_to, expected_rank=3)
    seq_length_to = input_shape_to[1]

    output = input_tensor
    output_to = input_tensor_to
    ...
    if use_token_type:
        ...
        flat_token_type_ids_to = tf.reshape(token_type_ids_to, [-1])
        one_hot_ids_to = tf.one_hot(flat_token_type_ids_to, depth=token_type_vocab_size)
        token_type_embeddings_to = tf.matmul(one_hot_ids_to, token_type_table)
        token_type_embeddings_to = tf.reshape(token_type_embeddings_to,
                                           [batch_size, seq_length_to, width])
        output_to += token_type_embeddings_to

    # if level_type_ids != None:
    #     # level_type
    #     level_type_table = tf.get_variable(
    #         name="level_type_embedding",
    #         shape=[3, width],
    #         initializer=create_initializer(initializer_range))
    #     flat_level_type_ids = tf.reshape(level_type_ids, [-1])
    #     level_one_hot_ids = tf.one_hot(flat_level_type_ids, depth=3)
    #     level_type_embeddings = tf.matmul(level_one_hot_ids, level_type_table)
    #     level_type_embeddings = tf.reshape(level_type_embeddings,
    #                                        [batch_size, seq_length, width])
    #     output += level_type_embeddings
    ...
    if use_position_embeddings:
        ...
        with tf.control_dependencies([assert_op]):
            ...
            position_embeddings_to = tf.slice(full_position_embeddings, [0, 0],
                                           [seq_length_to, -1])
            num_dims_to = len(output_to.shape.as_list())

            position_broadcast_shape_to = []
            for _ in range(num_dims_to - 2):
                position_broadcast_shape_to.append(1)
            position_broadcast_shape_to.extend([seq_length_to, width])
            position_embeddings_to = tf.reshape(position_embeddings_to,
                                             position_broadcast_shape_to)
            output_to += position_embeddings_to

    output = layer_norm_and_dropout(output, dropout_prob)
    output_to = layer_norm_and_dropout(output_to, dropout_prob)
    # output = layer_norm(output)
    # output = tf.keras.layers.SpatialDropout1D(dropout_prob)(output)
    return output, output_to
...
def _generate_relative_positions_embeddings(length, depth,
    ...
    # embeddings = tf.reshape(embeddings, [relative_positions_matrix.shape[0],relative_positions_matrix.shape[1],depth])
...
def transformer_model(input_tensor_from,
                      input_tensor_to,
                      attention_mask,
                      hidden_size=768,
                      num_hidden_layers=12,
    ...
    input_shape = get_shape_list(input_tensor_from, expected_rank=3)
    batch_size = input_shape[0]
    seq_length_from = input_shape[1]
    ...
    # help the optimizer.
    prev_output = reshape_to_matrix(input_tensor_from)

    layer_input_to = reshape_to_matrix(input_tensor_to)
